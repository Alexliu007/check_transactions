...
# 对比tronscan & trongrid 获得的txid 内容，对比其内部交易是否相同
def check_transactions(address,excel_path):
    ...
    transactions_info_tronscan_s = requests.get(base_url+address,headers=headers,timeout=50)
                    ...
                    if len(tran_info_from_trongrid) > 0 or len(tran_info_from_tronscan) > 0:
                        ...
                        if len(tran_info_from_trongrid) == len(tran_info_from_tronscan):
                            ...
                            if len(diff) > 0:
                                ...
                                print('txid:', txid)
                                print('from_trongrid',tran_info_from_trongrid)
                                print('from_tronscan',tran_info_from_tronscan)
...
# 通过tronscan 获取txid相关信息  
def get_transactions_from_tronscan(txid):
    base_url_tronscan = 'https://apilist.tronscan.org/api/transaction-info?hash='
    transactions_info_tronscan_s = requests.get(base_url_tronscan+txid,headers=headers,timeout=50)
    if transactions_info_tronscan_s.status_code == 200:
        transactions_info_tronscan = transactions_info_tronscan_s.json()
...
# 通过tronscan获得usdt holders地址
def check_address_info_from_tronscan(tronscan_url,excel_path):
    usdt_info_form_tronscan_content1 = requests.get(tronscan_url,headers=headers,timeout=50)
...
def main():
    ...
    # 初始化excel表
    #--------------
    path = "/Users/tron/Desktop/tran_check/tran_check_0423.xls" # excel表路径
